...
public class PersonClient {
  public static void main(String[] args) {
    ...
    try {
      ...
      PersonHome pHome = (PersonHome)context.lookup("ejb/CMP20-PersonHome");

      // First, see if this person already exists
      System.out.println("Looking for " + fname + " " + lname);
      Person person = null;
      try {
	person = pHome.findByPrimaryKey(fullName);
      }
      catch (Exception e) {
	System.out.println("Exception while looking up person.");
      ...
      if (person == null) {
	System.out.println("Creating person " + fullName);
	person = pHome.create(fullName, fname, lname);
      }

      // Get the person's profile
      Profile profile = person.getProfile();
      // Set some entries in the profile
      System.out.println("Setting profile entries for " + fullName);
      profile.setEntry("favoriteColor", "blue");
      profile.setEntry("language", "German");

      // Get the entries from the profile
      System.out.println("Getting profile entries for " + fullName);
